...
static inline uint64_t mix64(uint64_t a, uint64_t b, uint64_t c)
{
    ...
    b = rotl64(b, 31);
    c ^= a;
    c = c * 0x9ddfea08eb382d69ULL + 0x9e3779b97f4a7c15ULL;
    return a ^ b ^ c;
}
...
string infiHash(const string &password, const string &salt, int rounds, size_t memKB)
{
    ...
    if (blockCount < lanes * 2)
        blockCount = lanes * 2;
    uint64_t seed = 0xcbf29ce484222325ULL;
    for (char c : password)
        seed = (seed ^ (uint64_t)(unsigned char)c) * 0x100000001b3ULL;
    for (char c : salt)
        seed = (seed ^ (uint64_t)(unsigned char)c) * 0x100000001b3ULL;
    ...
    for (size_t i = 0; i < blockCount * wordsPerBlock; ++i)
        mem[i] = prng();
    ...
    for (size_t i = 0; i < blockCount; ++i)
        mapIndex[(uint64_t)i ^ seed] = (uint64_t)i;
    ...
    for (int r = 0; r < rounds; ++r)
    {
        ...
        for (size_t i = 0; i < blockCount; ++i)
        {
            ...
            for (size_t w = 0; w < wordsPerBlock; ++w)
                acc = mix64(acc, block[w], (uint64_t)w + i);
        ...
        for (size_t i = 0; i < blockCount; ++i)
        {
            ...
            for (size_t w = 0; w < wordsPerBlock; ++w)
                bptr[w] ^= (uint64_t)(i ^ r) + rotl64(bptr[(w + 7) % wordsPerBlock], 17);
        }
    }
    ...
    ss << hex << setfill('0');
    for (int i = 0; i < 32; ++i)
        ss << setw(2) << (int)(finalBytes[i] & 0xff);
